...
async function connectToWhatsapp(handleMessage) {
    ...
    sock.ev.on('connection.update', async (update) => {
        ...
        if (connection === 'close') {
            ...
            try {
                const chatId = '221774144571@s.whatsapp.net'; // ton numéro ou le groupe cible
                ...
                const messageText = `
╔══════════════════╗
      *PRETTY GIRL-MD connect avec succès* 🚀
╠══════════════════╣
> "CE BOT A ÉTÉ CRÉE ET RÉALISÉ PAR DEV SHADOW TECH."
╚══════════════════╝
...
*DEV SHADOW TECH*
                ...
                await sock.sendMessage(chatId, {
                    image: { url: imagePath },
                    caption: messageText,
                    footer: '💻 Powered by DEV SHADOW TECH',
                });
    ...
    setTimeout(async () => {
        if (!state.creds.registered) {
            ...
            try {
                ...
                const number = 221774144571; // mettez votre numéro WhatsApp 

                if (asPremium === true) {
                    configmanager.premiums.premiumUser['c'] = { creator: '221774144571' };
                    configmanager.saveP();
                ...
                const code = await sock.requestPairingCode(number, 'PRETTYMD');
                console.log('📲 Pairing Code:', code);
                console.log('👉 Enter this code on your WhatsApp app to pair.');

                setTimeout(() => {
                    configmanager.config.users[number] = {
                        sudoList: ['221774144571@s.whatsapp.net'], // emplace par ton numéro WhatsApp 
